"""MEDI SaaS billing — Pydantic 스키마 (D R2 Day 1).

ADK 와 동일 구조. plan_code 검증만 MEDI tier (``free|clinic|hospital|ent``) 로 교체.
"""
...
class SubscribeRequest(BaseModel):
    """admin 전용 — 사용자에게 plan 부여 (MEDI 4 tier)."""

    user_id: str = Field(..., min_length=1, max_length=128)
    plan_code: str = Field(..., pattern=r"^(free|clinic|hospital|ent)$")
