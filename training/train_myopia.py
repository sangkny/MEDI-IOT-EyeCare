#!/usr/bin/env python3
"""근시(Myopia) 단독 분류 — EfficientNet-B4 + Focal Loss.

- backbone: v4 가중치 초기화 (features/avgpool)
- head: binary (normal=0 / myopia=1)
- loss: Focal Loss (gamma, alpha) + WeightedRandomSampler
- 평가: AUC (주), F1, Sensitivity, Specificity
- 목표: val AUC >= 0.90

예:
  python training/train_myopia.py \\
    --manifest training/manifests/myopia_v1.json \\
    --pretrained models/retinal_v4.pt \\
    --output models/retinal_myopia_v1 \\
...
class MyopiaClassifier(nn.Module):
    """EfficientNet-B4 + binary myopia head."""
...
class FocalLoss(nn.Module):
    """Binary focal loss — positive class alpha (myopia) 보정."""
...
@torch.no_grad()
def eval_myopia(model: MyopiaClassifier, loader: DataLoader, device: torch.device) -> MyopiaMetrics:
    from sklearn.metrics import confusion_matrix, f1_score, roc_auc_score

    if len(loader.dataset) == 0:  # type: ignore[arg-type]
        return MyopiaMetrics(0.0, 0.0, 0.0, 0.0)
    ...
    return MyopiaMetrics(auc=auc, f1=f1, sensitivity=sensitivity, specificity=specificity)


def load_v4_backbone(model: MyopiaClassifier, path: Path) -> None:
...
def _myopia_label(entry: dict) -> int:
    return int(entry.get("myopia_grade", entry.get("label", 0)))


def _myopia_sample_weights(entries: list[dict]) -> list[float]:
    from collections import Counter

    labels = [_myopia_label(e) for e in entries]
...
def main() -> None:
    p = argparse.ArgumentParser(description="Myopia standalone training")
    p.add_argument("--manifest", type=Path, required=True)
    p.add_argument("--pretrained", type=Path, default=ROOT / "models" / "retinal_v4.pt")
    p.add_argument("--output", type=Path, default=ROOT / "models" / "retinal_myopia_v1")
    ...
    p.add_argument("--early-stop", dest="early_stop", type=int, default=15)
    ...
    train_loader = DataLoader(
        ...
        sampler=WeightedRandomSampler(
            weights=_myopia_sample_weights(train_entries),
    ...
    model = MyopiaClassifier(pretrained_imagenet=not pretrained_path.is_file())
    ...
    pos = sum(_myopia_label(e) for e in train_entries)
    print(
        f"myopia train={len(train_entries)} pos={pos} neg={len(train_entries)-pos} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scheduler.step()
        val_m = eval_myopia(model, val_loader, device)
        print(
    ...
    val_m = eval_myopia(model, val_loader, device)
    test_m = eval_myopia(model, test_loader, device)
