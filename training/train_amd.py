#!/usr/bin/env python3
"""AMD 단독 분류 — EfficientNet-B4 + Focal Loss.

- backbone: v4 가중치 초기화 (features/avgpool)
- head: binary (normal=0 / amd=1)
- loss: Focal Loss (gamma, alpha) + WeightedRandomSampler
- 평가: AUC (주), F1, Sensitivity, Specificity
- 목표: val AUC >= 0.88

예:
  python training/train_amd.py \\
    --manifest training/manifests/amd_v1.json \\
    --pretrained models/retinal_v4.pt \\
    --output models/retinal_amd_v1 \\
...
class AMDClassifier(nn.Module):
    """EfficientNet-B4 + binary AMD head."""
...
class FocalLoss(nn.Module):
    """Binary focal loss — positive class alpha (AMD) 보정."""
...
@torch.no_grad()
def eval_amd(model: AMDClassifier, loader: DataLoader, device: torch.device) -> AMDMetrics:
    from sklearn.metrics import confusion_matrix, f1_score, roc_auc_score

    if len(loader.dataset) == 0:  # type: ignore[arg-type]
        return AMDMetrics(0.0, 0.0, 0.0, 0.0)
    ...
    return AMDMetrics(auc=auc, f1=f1, sensitivity=sensitivity, specificity=specificity)


def load_v4_backbone(model: AMDClassifier, path: Path) -> None:
...
def _amd_label(entry: dict) -> int:
    return int(entry.get("amd_grade", entry.get("label", 0)))


def _amd_sample_weights(entries: list[dict]) -> list[float]:
    from collections import Counter

    labels = [_amd_label(e) for e in entries]
...
def main() -> None:
    p = argparse.ArgumentParser(description="AMD standalone training")
    p.add_argument("--manifest", type=Path, required=True)
    p.add_argument("--pretrained", type=Path, default=ROOT / "models" / "retinal_v4.pt")
    p.add_argument("--output", type=Path, default=ROOT / "models" / "retinal_amd_v1")
    ...
    train_loader = DataLoader(
        ...
        sampler=WeightedRandomSampler(
            weights=_amd_sample_weights(train_entries),
    ...
    model = AMDClassifier(pretrained_imagenet=not pretrained_path.is_file())
    ...
    pos = sum(_amd_label(e) for e in train_entries)
    print(
        f"amd train={len(train_entries)} pos={pos} neg={len(train_entries)-pos} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scheduler.step()
        val_m = eval_amd(model, val_loader, device)
        print(
    ...
    val_m = eval_amd(model, val_loader, device)
    test_m = eval_amd(model, test_loader, device)

    torch.save(
        {
            "model_state": model.state_dict(),
            "arch": "efficientnet_b4_amd",
