#!/usr/bin/env python3
"""
파일명: scripts/export_v15_onnx.py
목적: v15 6-head ONNX export (v10 5-head + glaucoma_grade)

예 (GPU):
  python3 scripts/export_v15_onnx.py \\
    --checkpoint models/retinal_v15/best.pt \\
    --output models/retinal_v15.onnx
"""
...
class V15OnnxWrapper(nn.Module):
    """6-head: dr·glaucoma·amd·myopia·multidisease·glaucoma_grade."""
    ...
    def forward(
        ...
        grade = torch.softmax(out["glaucoma_grade"], dim=-1)
        return dr, gl, amd, myo, multi, grade
...
def _verify_onnx(onnx_path: Path, image_size: int = 224) -> None:
    ...
    if len(outs) != 6:
        raise SystemExit(f"FAIL: expected 6 outputs, got {len(outs)}")


def main() -> None:
    p = argparse.ArgumentParser(description="Export v15 6-head ONNX")
    ...
    model = MultiTaskV10Model(pretrained_imagenet=False, grade_head=True)
    model.load_state_dict(_load_state(ckpt_path), strict=False)
    model.eval()
    wrapper = V15OnnxWrapper(model)
    ...
    meta.update(
        {
            "arch": "efficientnet_b4_v15",
            "preprocess": meta.get("preprocess") or "clahe",
