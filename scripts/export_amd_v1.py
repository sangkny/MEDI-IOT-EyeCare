#!/usr/bin/env python3
"""
파일명: export_amd_v1.py
목적: export_amd_v1.py 실행 스크립트
히스토리:
  2026-06-11 - 현재 상태 문서화 + 히스토리 추가

retinal_amd_v1 best.pt → ONNX + meta.json.
"""
...
from training.train_amd import AMDClassifier


def _map_state_for_export(state: dict) -> dict:
    """head.* 키는 AMDClassifier와 동일 — ONNX export용 그대로 사용."""
    out: dict = {}
...
def main() -> None:
    p = argparse.ArgumentParser(description="Export AMD v1 to ONNX")
    p.add_argument(
        "--checkpoint",
        type=Path,
        default=ROOT / "models" / "retinal_amd_v1" / "best.pt",
    )
    p.add_argument("--output", type=Path, default=ROOT / "models" / "retinal_amd_v1.onnx")
    ...
    model = AMDClassifier(pretrained_imagenet=False)
    ...
    meta = {
        ...
        "task": "amd",
        "manifest": src_meta.get("manifest") or "amd_v1.json",
